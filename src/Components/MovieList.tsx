...
export function MovieList({ category }: MovieListProps) {
  ...
  const bigMovieMatch: PathMatch<string> | null = useMatch(
    `${convertCategory(category)}/movies/:movieId`
  );
  ...
  const onBoxClicked = (movieId: number) => {
    const categoryPath = convertCategory(category);
    const url = categoryPath
      ? `/${categoryPath}/movies/${movieId}`
      : `/movies/${movieId}`;
    console.log("Generated URL:", url, "Category:", category);
    navigate(url);
  };
  const onOverlayClick = () => navigate(`/${convertCategory(category)}`);
  const clickedMovie =
